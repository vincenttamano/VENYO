...
public class BookingAdmin implements AdminManagement<Booking> {
    ...
    public void displayAll() {
        ...
        if (list.isEmpty()) {
            ...
            for (Booking b : list) {
                System.out.println("ID:" + b.getBookingId() + " | Venue:" + (b.getVenue() == null ? "N/A" : b.getVenue().getName()) + 
        ...
        switch (filterChoice) {
            case 1: filterStatus = BookingStatus.BOOKED; break;
            case 2: filterStatus = BookingStatus.PENDING; break;
            case 3: filterStatus = BookingStatus.CANCELLED; break;
            case 4: filterStatus = BookingStatus.FINISHED; break;
            default: filterStatus = null;
        }
    ...
    private LinkedList<Booking> loadAllBookings() {
        ...
        for (Document doc : collection.find()) {
            ...
            try { if (pay != null) pstat = PaymentStatus.valueOf(pay.toUpperCase()); } catch (Exception ignored) {}
            BookingStatus bstat = BookingStatus.PENDING;
            try { if (stat != null) bstat = BookingStatus.valueOf(stat.toUpperCase()); } catch (Exception ignored) {}
    ...
    private static Comparator<Booking> getHistoryComparator(int choice) {
        return (b1, b2) -> {
            ...
            if (choice == 1) { // finished first
                if (b1Finished && !b2Finished) return -1;
                if (!b1Finished && b2Finished) return 1;
                return 0;
    ...
    public void displayHistory(Scanner input) {
        ...
        if (sortChoice == 1 || sortChoice == 2) {
            ...
            for (Booking b : allHistory) {
                System.out.println("ID: " + b.getBookingId() + " | Status: " + b.getBookingStatus() + " | User: " + b.getUsername());
            }
        }
        ...
        switch (filterChoice) {
            case 1 -> finalList.stream().filter(b -> b.getBookingStatus() == BookingStatus.FINISHED).forEach(filteredList::add);
            case 2 -> finalList.stream().filter(b -> b.getBookingStatus() == BookingStatus.CANCELLED).forEach(filteredList::add);
